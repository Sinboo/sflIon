...
.controller('appCtrl', function ($rootScope, $state, $scope, $stateParams, appService, $ionicHistory, $ionicPopover, $ionicModal, listService, $wilddogArray, localStorageService, createWidget, updateWidget,
  ...
  listService.messages().$loaded(function (data) {
    $scope.messages = data;
  ...
  // var ref = new Wilddog('https://wxb.wilddogio.com/messages');
  // // 创建一个同步数组
  ...
  //
  // var createAssisFields = function (authData) {
  //   return {
  //     createAt: new Date().getTime(),
  //     createById: authData.uid.split(':')[1],
  //     updateAt: new Date().getTime(),
  //     updateById: authData.uid.split(':')[1]
  //   }
  // };
  // var authData;
  // var fatherRef = new Wilddog('https://wxb.wilddogio.com');
  // fatherRef.authWithPassword({email:"sinboo83@163.com",password:"wangxin"}, function(err,data){
  //   if(err == null){
  //     console.log("auth success!");
  //     authData = fatherRef.getAuth();
  ...
  //   console.log(newMessageText);
  //   $scope.messages.$add(
  //     angular.extend(createAssisFields(authData), {
  //       'text': newMessageText
  //     })
  //   )
  // };
  ...
  function initShop() {
    ...
    $ionicModal.fromTemplateUrl('templates/shop/product-preview.html', {
      scope: $scope,
      animation: 'fade-in-scale'
    }).then(function (modal) {
    ...
    $ionicModal.fromTemplateUrl('templates/shop/product-details.html', {
      scope: $scope,
      animation: 'jelly'
    }).then(function (modal) {
...
moment.locale('zh-cn', {
  calendar: {
    lastDay: '[昨天]',
    sameDay: '[今天]',
    nextDay: '[明天, ] dddd Do MMM',
    lastWeek: '[上个] dddd Do MMM',
