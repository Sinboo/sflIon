  ...
  .controller('ReceptionistCreateEditReservationCtrl', function ($scope, $rootScope, $state, $timeout, noBackGoTo, appModalService, listService, UID, UserProfile, RESERVATION_TIME_LIST, $filter, ionicToast, $interval, JoinList) {
    ...
    var initData = function () {
      ...
      if (!$rootScope['orderOfHairstylistLoadedFlagReceptionist-' + ($scope.reservation.hairstylistUid || $scope.reservation.hairstylist.uid)]) {
        $scope.orderList.$loaded().then(function (data) {
          ...
          $rootScope['orderOfHairstylistLoadedFlagReceptionist-' + ($scope.reservation.hairstylistUid || $scope.reservation.hairstylist.uid)] = true;
        });
    ...
    $scope.saveReservation = function (reservation) {
      ...
      if (!$scope.editFlag) {
        ...
        // postData.customerUid = UID(); 这里应该是用户扫码注册后的uid
        ...
        list.$loaded().then(function () {
          var index = list.$indexFor($state.params.reservation.$id);
