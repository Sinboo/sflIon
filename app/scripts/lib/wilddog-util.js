/*!
 * Wilddog-util:  A set of experimental power tools for Wilddog.
 *
 * Version: 0.0.0
 * URL: https://github.com/Wilddog/Wilddog-util
 * Date: 2015-09-23T15:52:25.612Z
 * License: MIT http://Wilddog.mit-license.org/
 */

require=(function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '"+o+"'");throw f.code="MODULE_NOT_FOUND",f}var l=n[o]={exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e)},l,l.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(require,module,exports){
(function (global){
// place fbutil onto Wilddog.util namespace
// if we are in the browser and Wilddog exists
if( global.Wilddog ) {
  global.Wilddog.util = require('Wilddog-util');
}
}).call(this,typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {})
},{"Wilddog-util":undefined}],2:[function(require,module,exports){
 ...
 *    - RecordSet: The root of a normalized collection; a set of merged Wilddog paths
 *    - Record: A child of RecordSet obtained by calling .child(recordId)
 *    - RecordField: A child of Record or RecordField which represents a wrapped Wilddog path (no longer normalized)
 *
 ...
 * of reverting to the underlying Wilddog instances.
 *
...
AbstractRecord.prototype = {
  ...
  /**
   * Returns a Wilddog URL or a merged set of URLs for the Record
   * @return {String}
   */
  getUrl: function() {
    return this._url;
...
FieldMap.prototype = {
   ...
   * it will return null.
   *
   * @param {Array} snaps a list of Wilddog snapshots
  ...
  add: function(pathProps) {
    ...
    if( util.contains(this.pathNames, path.name()) ) {
      throw new Error('Duplicate path name. The .key() value for each path must be unique, or you ' +
          'can give each a path an alias by using [WilddogRef, alias] in the constructor. The aliases ' +
          'must also be unique.');
    }
    this._map(path);
...
util.registerWilddogWrapper(Query);
 ...
 * It is used by NormalizedRef to create snapshots and monitor Wilddog for data changes.
 *
 * A RecordSet represents the root level of NormalizedCollection's output. It is a list of
 * collections (from multiple paths in Wilddog) to be joined together.
 *
 * This is, for the purposes of a NormalizedCollections, the root of the data. Calls to parent()
 * from here should return null, just like they would from the root of a Wilddog. This is because
...
function RecordSet(fieldMap, whereClause) {
  ...
  // the RecordSetEventManager handles Wilddog events and calls event handlers on
...
exports.Scroll = function(baseRef, sortField, opts) {
  if( !util.isWilddogRef(baseRef) || util.isQueryRef(baseRef) ) {
    throw new Error('First argument to Wilddog.util.Scroll must be a valid Wilddog ref. It cannot be a Query (e.g. you have called orderByChild()).');
  }

  if( typeof sortField !== 'string' ) {
    throw new Error('Second argument to Wilddog.util.Scroll must be a valid string');
  }
  if( arguments.length > 2 && !util.isObject(opts) ) {
    throw new Error('Optional third argument to Wilddog.util.Scroll must be an object of key/value pairs');
  }
...
exports.Paginate = function(baseRef, sortField, opts) {
  if( !util.isWilddogRef(baseRef) || util.isQueryRef(baseRef) ) {
    throw new Error('First argument to Wilddog.util.Paginate must be a valid Wilddog ref. It cannot be a Query (e.g. you have called orderByChild()).');
  }
  if( typeof sortField !== 'string' ) {
    throw new Error('Second argument to Wilddog.util.Paginate must be a valid string');
  }
  if( arguments.length > 2 && !util.isObject(opts) ) {
    throw new Error('Optional third argument to Wilddog.util.Paginate must be an object of key/value pairs');
  }
  var ref = new ReadOnlyRef(baseRef);
...
util.undef = undef;
util.Wilddog = global.Wilddog || require('Wilddog');
...
var wrappingClasses = [];
util.isWilddogRef = function(x) {
  // ES5 throws TypeError if getPrototypeOf() called on a non-object
  if( !util.isObject(x) ) {
    return false;
  }

  // necessary because instanceof won't work on Wilddog Query objects
  // so we can't simply do instanceof here
  var proto = Object.getPrototypeOf(x);
  if( proto && proto.constructor === util.Wilddog.prototype.constructor ) {
    return true;
  }
  ...
  return util.isWilddogRefWrapper(x);
};

util.isWilddogRefWrapper = function(x) {
...
util.isQueryRef = function(x) {
  return util.isWilddogRef(x) && typeof x.transaction !== 'function';
};

// Add a Class as a valid substitute for a Wilddog reference, so that it will
// pass util.isWilddogRef(). This means that it must provide all the Wilddog
// API methods and behave exactly like a Wilddog instance in all cases.
util.registerWilddogWrapper = function(WrappingClass) {
  wrappingClasses.push(WrappingClass);
};

// for test units
util._mockWilddogRef = function(mock) {
  util.Wilddog = mock;
};
...
},{"Wilddog":"Wilddog"}],"Wilddog-util":[function(require,module,exports){
/**
 * This file establishes the Wilddog.util namespace and
...
if( typeof window !== 'undefined' ) {
  if( !window.hasOwnProperty('Wilddog') ) {
    console.warn('Wilddog not found on the global window instance. Cannot add Wilddog.util namespace.');
  }
  else {
    window.Wilddog.util = util;
  }
}
